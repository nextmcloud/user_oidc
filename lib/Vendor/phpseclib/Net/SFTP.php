 ...
 *
 *    $sftp = new \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP('www.domain.tld');
...
class SFTP extends SSH2
{
     ...
     *
     * \OCA\UserOIDC\Vendor\phpseclib\Net\SSH2::exec() uses 0 and \OCA\UserOIDC\Vendor\phpseclib\Net\SSH2::read() / \OCA\UserOIDC\Vendor\phpseclib\Net\SSH2::write() use 1.
     *
    ...
    /**#@+
     * @access public
     * @see \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::put()
    */
    /**
     ...
     * see \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::_parseLongname() for an explanation
     *
     * @var array
     */
     ...
     * @param int $port
     * @param int $timeout
     * @return \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP
     * @access public
     */
    function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        // http://tools.ietf.org/html/draft-ietf-secsh-filexfer-13#section-7.1
        // the order, in this case, matters quite a lot - see \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::_parseAttributes() to understand why
    ...
    function _init_sftp_connection()
    {
        ...
        /*
         ...
         in draft-ietf-secsh-filexfer-13 would be quite impossible.  As such, what \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP would do is close the
    ...
    function chdir($dir)
    {
        ...
        // see \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::nlist() for a more thorough explanation of the following
        $response = $this->_get_sftp_packet();
        switch ($this->packet_type) {
     ...
     * Returns general information about a file or symbolic link
     *
     * Determines information without calling \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::realpath().
    ...
    function chown($filename, $uid, $recursive = false)
    {
        /*
         ...
         OCA\UserOIDC\Vendor\phpseclib _could_ auto append the dns_domain to $uid BUT what if it shouldn't
         have one? OCA\UserOIDC\Vendor\phpseclib would have no way of knowing so rather than guess OCA\UserOIDC\Vendor\phpseclib
     ...
     *
     * By default, \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::put() does not read from the local filesystem.  $data is dumped directly into $remote_file.
     * So, for example, if you set $data to 'filename.ext' and then do \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::get(), you will get a file, twelve bytes
     ...
     * @return bool
     * @access public
     * @internal ASCII mode for SFTPv4/5/6 can be supported by adding a new function - \OCA\UserOIDC\Vendor\phpseclib\Net\SFTP::setMode().
     */
